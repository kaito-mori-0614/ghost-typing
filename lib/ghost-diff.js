...
function protectedPrefixLength(expected) {
  return leadingWhitespace(expected).length;
}

function protectedPrefixRepair(actual, expected) {
  const targetPrefix = leadingWhitespace(expected);
  if (!targetPrefix) return null;

  const current = String(actual ?? '');
  if (current.startsWith(targetPrefix)) return null;

  const currentPrefix = leadingWhitespace(current);
  return {
    start: 0,
    end: currentPrefix.length,
    text: targetPrefix
  };
}
